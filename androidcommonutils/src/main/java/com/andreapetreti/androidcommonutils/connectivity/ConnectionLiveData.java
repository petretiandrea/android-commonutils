package com.andreapetreti.androidcommonutils.connectivity;
...
/**
 * ConnectionLiveData allow to observe the status of Mobile and WiFi connections, it automatically
 * update the observers to all network changes.
 */
public class ConnectionLiveData extends LiveData<Boolean> {
    ...
    public static ConnectionLiveData newInstance(Context context) {
        return new ConnectionLiveData(context);
    }

    @RequiresPermission(android.Manifest.permission.ACCESS_NETWORK_STATE)
    protected ConnectionLiveData(Context context) {
        mContext = context;
